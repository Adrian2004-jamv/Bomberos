...
Para vaciar el padron de emergencias de prueba, defina `DJANGO_LIMPIAR_EMERGENCIAS_ANTES_DE` con una fecha `AAAA-MM-DD` y espere la reconstruccion: se retiran las emergencias reportadas antes de ese dia con sus formularios SCI, sus despliegues y sus posiciones de GPS, y las unidades que estaban asignadas vuelven al inventario como disponibles. El inventario en si no se toca. La variable lleva la fecha y no un `1` a proposito: sin corte no hay borrado, de modo que dejarla puesta por descuido no arrasa con lo que se registre despues; aun asi conviene retirarla en cuanto termine el despliegue. En local el comando es `python manage.py limpiar_emergencias`, que ensaya salvo que se le pase `--ejecutar` y admite `--antes-de`, `--codigo`, `--estacion` o `--todas`.

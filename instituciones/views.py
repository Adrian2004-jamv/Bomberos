...
def cuerpos_visibles_para(usuario):
...
@login_required
def lista(request):
    cuerpos = cuerpos_visibles_para(request.user)
    ...
    return render(request, "instituciones/lista.html", contexto)
...
def detalle(request, pk):
    ...
    cuerpo = get_object_or_404(
        cuerpos_visibles_para(request.user).prefetch_related(
    ...
    return render(request, "instituciones/detalle.html", contexto)
...
def crear_cuerpo(request):
    ...
    contexto = {"form": form, "titulo": "Crear Cuerpo de Bomberos"}

    return render(request, "instituciones/formulario_cuerpo.html", contexto)
...
def editar_cuerpo(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect("instituciones:detalle", pk=cuerpo.pk)
    contexto = {"form": form, "cuerpo": cuerpo, "titulo": "Editar Cuerpo de Bomberos"}
...
def crear_estacion(request, cuerpo_pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect("instituciones:detalle", pk=cuerpo.pk)
    contexto = {"form": form, "cuerpo": cuerpo, "titulo": "Crear estación"}

    return render(request, "instituciones/formulario_estacion.html", contexto)
